...
def get_new_experiment_dir(config, data_flag, root="logs", model=''):
...
if __name__ == "__main__":
    ...
    logging_dir = get_new_experiment_dir(config, data_flag, model='vnet')
    ...
    model = create_model(
        name='vnet', 
        n_channels=config.slim_unetr.in_channels,
        num_classes=config.slim_unetr.out_channels,
        n_filters=16
    )
    ...
    base_exp_path_save = get_new_experiment_dir(config, data_flag, root="model_store", model='vnet')
